...
def main(
    cfg: "DictConfig",
    make_environment_fn=make_environment,
    make_sac_agent_fn=make_sac_agent,
    ...
    # Create environments
    train_env, eval_env = make_environment_fn(cfg, logger=logger)

    # Create agent
    model, exploration_policy = make_sac_agent_fn(cfg, train_env, eval_env, device)

    # Create SAC loss
    loss_module, target_net_updater = make_loss_module_fn(cfg, model)

    # Create off-policy collector
    collector = make_collector_fn(cfg, train_env, exploration_policy)

    # Create replay buffer
    replay_buffer = make_replay_buffer_fn(
    ...
    (
        ...
        optimizer_critic,
        optimizer_alpha,
    ) = make_sac_optimizer_fn(cfg, loss_module)
